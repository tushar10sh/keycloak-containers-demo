...
Then build the image with:
    ...
    sudo docker build -t demo-keycloak -f keycloak/Dockerfile .

Finally run it with:

    sudo docker run --name demo-keycloak -e KEYCLOAK_USER=admin -e KEYCLOAK_PASSWORD=admin \
        -p 5200:8080 --net demo-network demo-keycloak
...
First build the image with:
    ...
    sudo docker build -t demo-ldap ldap
    
Then run it with:

    sudo docker run --name demo-ldap --net demo-network demo-ldap
...
Start the mail server with:
    ...
    sudo docker run -d -p 8025:8025 -p 1025:1025  --name demo-mail --net demo-network mailhog/mailhog
...
First build the image with:
    If Ip of Host is 192.168.0.100 
    Auth url of KEYCLOAK: http://keycloak-192-168-0-100.nip.io:5200/auth 
    Modify src/keycloak.json with above url
    Modify src/index.html with keycloak.js path as : http://keycloak-192-168-0-100.nip.io:5200/auth/js/keycloak.js
    Now run following to build container
    sudo docker build -t demo-js-console js-console
    
Then run it with:

    sudo docker run --name demo-js-console -p 5201:80 demo-js-console
    Access via : http://js-console-192-168-0-100.nip.io:5201/
...
* Valid Redirect URIs: `http://js-console-192-168-0-100.nip.io:5201/*`
* Web Origins: `http://js-console-192-168-0-100.nip.io:5201`
...
* Host: `demo-mail`
* From: `keycloak-demo-realm@avid.sac.isro.gov.in`

Click `Save` and `Test connection`. Open your http://mailhog-192-168-0-100.nip.io:8025/ and check that you have
...
Open the [Keycloak Admin Console](http://keycloak-192-168-0-100.nip.io:5200/auth/admin/). 
...
To try this out open the [JS Console](http://js-console-192-168-0-100.nip.io:5201). 
...
Go back to the [JS Console](http://js-console-192-168-0-100.nip.io:5201) and click `Login` again. Now Keycloak will prompt the user to grant 
...
Go back to the [JS Console](http://js-console-192-168-0-100.nip.io:5201) and click `Refresh`, then `Access Token JSON`.
Notice that there is a `realm_access` claim in the token that now contains the user role.

Next let's create a Group. Go back to the [Keycloak Admin Console](http://keycloak-192-168-0-100.nip.io:5200/auth/admin/).
...
Try opening the [JS Console](http://js-console-192-168-0-100.nip.io:5201) again and login with one of
these users.
...
Try opening the [JS Console](http://js-console-192-168-0-100.nip.io:5201) again and instead of providing
...
Try opening the [JS Console](http://js-console-192-168-0-100.nip.io:5201) to login a take in the beauty of the
new login screen!
...
First let's see what algorithm is currently in use. Open the [JS Console](http://js-console-192-168-0-100.nip.io:5201), 
...
In a different tab open the [JWT validation extension](http://keycloak-192-168-0-100.nip.io:5200/auth/realms/demo/jwt). 
...
Open the [Keycloak Admin Console](http://keycloak-192-168-0-100.nip.io:5200/auth/admin/) in a new tab. Make sure you
...
Click on `ID Token`, copy it and open the [JWT validation extension](http://keycloak-192-168-0-100.nip.io:5200/auth/realms/demo/jwt)
...
Go back to the [Keycloak Admin Console](http://keycloak-192-168-0-100.nip.io:5200/auth/admin/). Go to
...
Now go back to the JS Console and clik on `Refresh`. Copy the token to the [JWT validation extension](http://keycloak-192-168-0-100.nip.io:5200/auth/realms/demo/jwt).
...
Make sure you have the [JS Console](http://js-console-192-168-0-100.nip.io:5201) open in a tab and you're logged-in.
Open the [Keycloak Admin Console](http://keycloak-192-168-0-100.nip.io:5200/auth/admin/) in another tab.

Find the user you are logged-in as and click on `Sessions`. Click on `Logout all session`.

Go back to the [JS Console](http://js-console-192-168-0-100.nip.io:5201) and click `Refresh`. Notice how you are 
...
Open the [Keycloak Admin Console](http://keycloak-192-168-0-100.nip.io:5200/auth/admin/). Click on `Events`
and `Config`. Turn on `Save Events` and click `Save`.

Go back to the [JS Console](http://js-console-192-168-0-100.nip.io:5201) and click `Refresh`. Logout. Then
when you log in use the wrong password, then login with the correct password.

Go back to the `Events` in the [Keycloak Admin Console](http://keycloak-192-168-0-100.nip.io:5200/auth/admin/)
...
To enable this open the [Keycloak Admin Console](http://keycloak-192-168-0-100.nip.io:5200/auth/admin/). 
...
Open the [JS Console](http://js-console-192-168-0-100.nip.io:5201) and click Logout. For the email enter your email
...
Now let's add WebAuthn to the mix. Open the [Keycloak Admin Console](http://keycloak-192-168-0-100.nip.io:5200/auth/admin/).
...
Open the [JS Console](http://js-console-192-168-0-100.nip.io:5201) and click Logout. Login again. After you've done the
